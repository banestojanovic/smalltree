        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button size="circle" variant="secondary">
                    <span>
                        <svg className={'size-4'} viewBox="0 0 18 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                ...
                <div className={`mt-4 space-y-6`}>
                    <section className={'flex w-5/6 items-center gap-3'}>
                        {product.cover?.original_url && (
                            <figure className={`flex size-16 shrink-0 items-center justify-center rounded-full border bg-white`}>
                                <img className="size-12 rounded-full object-cover" src={product.cover.original_url} alt="" />
                            </figure>
                        )}
                        <Typography as="h3"> {product.name}</Typography>
                    </section>
                    <section>
                        <Typography as="p" className="text-[15px]/7">
                            {product.description}
                        </Typography>
                    </section>
                    <section className={`space-y-2`}>
                        ...
                        <div>
                            <ToggleGroup type="single" variant="outline" className="justify-start" value={selectedVariation ?? undefined} onValueChange={(value) => setSelectedVariation(value)}>
                                {Object.keys(product?.grouped_variations ?? {}).map((group) => (
                                    <div key={group} className={'flex flex-col space-y-1'}>
                                        <Typography as="p" className={`text-xs/7`}>
                                            {' '}
                                            {group}{' '}
                                        </Typography>
